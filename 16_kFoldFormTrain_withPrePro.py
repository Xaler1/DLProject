...
for fold, (train_ids, test_ids) in enumerate(kfold.split(train_dataset)):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size = 256, sampler = train_subsampler)
    cvtest_loader = torch.utils.data.DataLoader(train_dataset, batch_size = 256, sampler = test_subsampler)
    ...
    for epoch in range(epochs):
        for i, (signal, labels) in enumerate(train_loader):
            ...
            if t <= T_0:
                lr = 10**(-6) + (t/T_0)*lr_max  
            else: 
                lr = lr_max*np.cos((np.pi/2)*((t-T_0)/(T_max-T_0))) + 10**(-7) 
        ...
        test_auc = 0
        with torch.no_grad():
            for i, (signal, labels) in enumerate(cvtest_loader):
            ...
            print(f"Epoch:  {epoch}/{epochs}  |  Train loss: {loss.item():.4f}  |  Train AUC: {train_AUC:.4f}  |  CVTest AUC: {test_auc}")
            print("___________________________________________________\n")
        ...
        if epoch%2 ==0:
            with open(f'{model_name}_fold{fold}_{seq_len}SeqLenModel.pth', 'wb') as f:
                pickle.dump(model, f)
            with open(f'{model_name}_fold{fold}_{seq_len}losses.pickle', 'wb') as f:
