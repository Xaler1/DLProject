...
class BasicConv1d(nn.Sequential):
    ...
    def __init__(self, filters:List[int] = [128, 128, 128, 128], kernel_size: int = 3, stride: int = 2, dilation : int = 1, 
                 pool_kernel: int = 1, pool_stride:int = 1, num_classes: int = 2, input_channels: int = 8, bn: bool = True, drop_p: float = 0.):
        layers = []
        for i in range(len(filters)):
            ...
            if pool_kernel > 0 and i < len(filters)-1:
                inner_layer.append(nn.MaxPool1d())
